...
| 会话劫持 | HttpOnly `hill_session` Cookie（JWT HS256）+ JTI 黑名单；拒绝 `alg=none` / 非 HS256；Bearer 仍可用于 API |
...
| 元数据/预览越权 | `GET /api/files/:hash` 与 `GET /api/files/:hash/thumb` 均校验属主或 admin（公开原图直链仍可匿名访问） |
| 安全响应头 | `X-Content-Type-Options`、`X-Frame-Options`、`Referrer-Policy` 等 |
| 配置写保护 | 管理端配置更新仅允许白名单 key；敏感字段 `***` / 空值表示不修改 |
| CSRF | 双提交 Cookie：`hill_csrf` + `X-CSRF-Token`（仅 cookie 会话写操作） |
| 跨用户去重 | 内容哈希按 **用户** 唯一；跨用户可复用 blob，但各自拥有独立归属行 |
